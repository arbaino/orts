<?php
    ...
    $HistoryList=DBConnect::getInstance()->getHistoryListByKeyword($_GET['keyword']);
    $step = 0;
    if(sizeof($HistoryList) > 0){
        $lastStatus = $HistoryList[(sizeof($HistoryList)-1)]['status_to'];
        //var_dump($lastStatus);exit;
    }
    if($lastStatus == 'Request'){
        $step = 0;
    }else if($lastStatus == 'Waiting for customer'){
        $step = 1;
    }else if($lastStatus == 'Waiting for repair'){
        $step = 2;
    }else if($lastStatus == 'Under repair'){
        $step = 3;
        ...
        <div class="well">
            ...
            <div class="order-status">
                ...
                <div class="order-status-timeline">
                    ...
                    <div class="order-status-timeline-completion c<?=$step?>"></div>
                </div>
                ...
                <div class="image-order-status image-order-status-active active img-circle">
                    <span class="status">Waiting for customer</span>
                    <div class="icon"></div>
                </div>
                <div class="image-order-status image-order-status-intransit active img-circle">
                    <span class="status">Waiting for repair</span>
                    <div class="icon"></div>
                </div>
                <div class="image-order-status image-order-status-delivered active img-circle">
                    <span class="status">Under repair</span>
                    <div class="icon"></div>
                </div>
